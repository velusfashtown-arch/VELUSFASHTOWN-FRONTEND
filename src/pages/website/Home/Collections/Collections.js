...
function CollectionCard({ collection, index }) {
  ...
  return (
    <Link
      ref={cardRef}
      className={`group relative block min-h-[438px] overflow-hidden bg-[#8b5145] text-left text-white no-underline max-[900px]:min-h-[350px] max-[620px]:min-h-[270px] reveal reveal-up ${isVisible ? 'reveal-visible' : ''}`}
...
export default function Collections() {
  ...
  return (
    <section className="py-[clamp(65px,8vw,120px)] px-[7vw] max-[620px]:px-[18px] max-[620px]:py-[61px]" id="collections">
      <div
        ref={headerRef}
        className={`text-center mb-[38px] max-[620px]:mb-[25px] reveal reveal-up ${headerVisible ? 'reveal-visible' : ''}`}
      >
