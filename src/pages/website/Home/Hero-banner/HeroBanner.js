...
export default function HeroBanner() {
  ...
  return (
    ...
    >
        ...
        {heroSlides.map((slide, i) => (
          <div key={i} className={`absolute inset-0 transition-opacity duration-[800ms] ease-in-out ${i === slideIndex ? 'opacity-100' : 'opacity-0'}`}>
            <img
              src={slide.image}
              alt={slide.eyebrow}
              className="h-full w-full object-cover object-[62%_center] max-[620px]:object-[64%_center]"
            />
            <div className="absolute -z-[2] inset-0" style={{ background: slide.shade }} />
          </div>
        ))}
      </div>
      <div className="absolute left-[clamp(24px,9vw,150px)] top-1/2 max-w-[455px] -translate-y-1/2 max-[620px]:bottom-[77px] max-[620px]:top-auto max-[620px]:max-w-[310px] max-[620px]:translate-y-0">
        <p className="m-0 mb-3 text-[#ffe6c7] font-bold text-[10px] tracking-[.19em]">{heroSlides[slideIndex].eyebrow}</p>
        <h1
          className="m-0 font-playfair font-medium tracking-[-.055em] leading-[.95] text-[clamp(48px,5vw,78px)] max-md:text-[clamp(40px,4.5vw,58px)] max-[620px]:text-[49px] max-[430px]:text-[38px]"
          dangerouslySetInnerHTML={{ __html: heroSlides[slideIndex].heading }}
        />
        <p className="max-w-[385px] my-6 mx-0 text-[14px] leading-[1.7] text-[rgba(255,250,245,.9)] max-[620px]:my-[17px] max-[620px]:text-[13px] max-[620px]:leading-[1.6]">{heroSlides[slideIndex].blurb}</p>
        <Link
      ...
      <div className="absolute bottom-[48px] left-[clamp(24px,9vw,150px)] z-[5] flex gap-[10px] max-[620px]:bottom-[39px]">
        {heroSlides.map((_, i) => (
          <button
            key={i}
            className={`w-[10px] h-[10px] rounded-full border-2 border-[rgba(255,255,245,.7)] bg-transparent p-0 cursor-pointer transition-all duration-[250ms] ${i === slideIndex ? 'bg-[rgba(255,255,245,.9)] scale-110' : 'hover:scale-110'}`}
            onClick={() => goToSlide(i)}
            aria-label={`Go to slide ${i + 1}`}
          />
        ))}
      </div>
      <div className="absolute bottom-[28px] right-[4.5vw] flex gap-[13px] items-center text-[9px] tracking-[.18em] font-bold max-[620px]:right-[21px] max-[620px]:bottom-[24px] max-[620px]:text-[8px]">
        <span className="opacity-65">0{slideIndex + 1}</span>
        <span>{heroSlides[slideIndex].note}</span>
      </div>
      {isPaused && (
