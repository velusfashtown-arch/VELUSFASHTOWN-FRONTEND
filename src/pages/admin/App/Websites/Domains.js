  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={handleAdd} disabled={saving || !newDomain.trim()} className="shrink-0 rounded-lg bg-terra px-4 py-2 text-xs font-bold uppercase tracking-wide text-white hover:bg-wine disabled:opacity-60">
          + Add
        </button>
      </div>
      ...
      {domains.length === 0 ? (
        ...
        <div className="space-y-2">
          {domains.map((d) => (
            <div key={d._id} className="flex items-center justify-between rounded-lg border border-line bg-paper p-3">
              <div className="min-w-0">
                <p className="flex items-center gap-2 text-sm font-semibold text-ink">
                  {d.domain}
                  {d.isPrimary ? <span className="rounded bg-terra/10 px-2 py-0.5 text-[10px] font-bold uppercase text-terra">Primary</span> : null}
                </p>
              ...
              <div className="flex shrink-0 items-center gap-1">
                {!d.isPrimary ? (
                  <button type="button" onClick={() => handleSetPrimary(d._id)} className="rounded border border-line px-2 py-1 text-xs font-semibold text-terra hover:bg-cream">Set Primary</button>
                ) : null}
                <button type="button" onClick={() => handleRemove(d._id)} className="rounded border border-red-200 px-2 py-1 text-xs font-semibold text-red-600 hover:bg-red-50">Remove</button>
              </div>
            </div>
          ))}
        </div>
