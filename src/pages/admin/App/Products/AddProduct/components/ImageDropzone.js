    ...
    <div className="flex flex-col gap-3">
      ...
      <div
        ...
        className={`relative border-2 border-dashed rounded-xl p-6 text-center transition-all duration-200 cursor-pointer
          ...
          ${uploading || remaining <= 0 ? 'opacity-50 cursor-not-allowed' : 'border-gray-200 hover:border-terra/50 hover:bg-terra/[0.02]'}`}
      >
        <input {...getInputProps()} />
        <div className="flex flex-col items-center gap-2">
          {uploading ? (
            ...
            <div className={`w-14 h-14 rounded-full ${isDragActive ? 'bg-terra/10' : 'bg-gray-100'} flex items-center justify-center`}>
              <FiUpload className={`w-6 h-6 ${isDragActive ? 'text-terra' : 'text-gray-400'}`} />
            </div>
          )}
          <div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-semibold text-gray-500">
                ...
                <span className="font-normal text-gray-400 ml-1">— Drag to reorder, first is thumbnail</span>
              </span>
            </div>
                ...
                <Reorder.Item key={url} value={url} className="relative">
                  <motion.div
                    ...
                    className={`relative group w-[100px] h-[133px] rounded-lg overflow-hidden border-2 cursor-grab active:cursor-grabbing
                      ${index === 0 ? 'border-terra shadow-md shadow-terra/10' : 'border-gray-200'}`}
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors duration-150 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                      <button
                        ...
                        title="Preview"
                      >
                        <FiImage className="w-3.5 h-3.5 text-gray-700" />
                      </button>
                      {index !== 0 && (
                        <button
                          type="button"
